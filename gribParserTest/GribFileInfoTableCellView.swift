...
class GribFileInfoTableCellView: NSTableCellView {
    ...
    private func getCoord(geography: GribGeographyData) -> String {
        if geography.rotated {
            ...
            return "Rotated grid: \(geography.latitudeOfFirstGridPointInDegrees)°N\(geography.longitudeOfFirstGridPointInDegrees)°E to \(geography.latitudeOfLastGridPointInDegrees)°N\(geography.longitudeOfLastGridPointInDegrees)°E with resolution \(geography.iDirectionIncrementInDegrees)°x\(geography.jDirectionIncrementInDegrees)°, actual grid: \(regCoordFirst.lat.twoDecimals)°N\(regCoordFirst.lon.twoDecimals)°E to \(regCoordLast.lat.twoDecimals)°N\(regCoordLast.lon.twoDecimals)°E"
        }
        return "Regular grid: \(geography.latitudeOfFirstGridPointInDegrees)°N\(geography.longitudeOfFirstGridPointInDegrees)°E to \(geography.latitudeOfLastGridPointInDegrees)°N\(geography.longitudeOfLastGridPointInDegrees)°E with resolution \(geography.iDirectionIncrementInDegrees)°x\(geography.jDirectionIncrementInDegrees)°"
    }
}
