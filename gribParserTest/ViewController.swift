...
//           let parse = try Parse(file: NSHomeDirectory() + "/MESAN_201812260000+000H00M")
            let parse = try GribParser(file: NSHomeDirectory() + "/Temp/MesanTemp/RCA_196101010000+000H00M")
            ...
            if let oneDataseries = parse.getValues(lons: lons, lats: lats, for: [parse.parameterList[0]]) {
                ...
                print(GribCoordinate.init(lon: 6.032269, lat: 52.81264, geography: parse.geographyData))
